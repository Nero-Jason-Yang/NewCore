...
- (void)storeDiskImageFromPath:(NSString *)path forKey:(NSString *)key
{
    if (path && key) {
        if (![_fileManager fileExistsAtPath:_diskCachePath]) {
            [_fileManager createDirectoryAtPath:_diskCachePath withIntermediateDirectories:YES attributes:nil error:NULL];
        ...
        if (![_fileManager fileExistsAtPath:path]) {
            // Notice: After application updated, AppID may changed, so stored path for last version may become invalid,
            // To resolve such an issue we correct the path using the new AppID.
            
            // Retrieve AppID (and its index in a valid path).
            static NSString *appID = nil;
            static int index = 0;
            if (!appID) {
                NSArray *comps = [_diskCachePath componentsSeparatedByString:@"/"];
                for (int i = 0; i < comps.count; i ++) {
                    if ([comps[i] isEqualToString:@"Applications"]) {
                        index = i + 1;
                        appID = comps[index];
                        break;
                    }
            ...
            // Correct path by replace AppID with new one.
            if (appID) {
        ...
        [_fileManager moveItemAtPath:path toPath:[self defaultCachePathForKey:key] error:nil];
...
- (void)storeDiskImageWithData:(NSData *)data forKey:(NSString *)key
{
    if (data && key) {
        if (![_fileManager fileExistsAtPath:_diskCachePath]) {
            [_fileManager createDirectoryAtPath:_diskCachePath withIntermediateDirectories:YES attributes:nil error:NULL];
...
- (NSString *)pathOfDiskImageForKey:(NSString *)key
{
...
- (BOOL) diskImageExistsWithKeyWithoutBlock:(NSString *)key {
...
- (void)removeImageForKey:(NSString *)key fromDisk:(BOOL)fromDisk {
    ...
    if (fromDisk) {
        [self removeDiskImageForKey:key];
    }
}

- (void)removeDiskImageForKey:(NSString *)key
{
    dispatch_async(self.ioQueue, ^{
